...
> 
> 기존 코드에서는 키보드 입력에 대한 Interrupt 처리를 한 후에 `IRQ_HANDLED`를 반환하지만, 우리가 고친 코드에서는 키보드 입력에 대한 Interrupt처리없이 완료가 되므로 아무런 문자가 입력되지 않는 것이다.
...
> **원인**
> 
> basic_setup()전에는 Keyboard Interrupt를 받을 수 있는 기본적인 Setting도 안되어 있을 것이라고 추측된다.
...
> 
> 아마 `atkbd_interrupt()`함수에서 input_sync(dev);에서 실제 monitor에 출력하도록 하는 것이 아닐까 싶다.
